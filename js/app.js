...
function renderMission(day) {
  ...
  if (done) {
    const data = DB.getDay(day).data; APP.answered = true; APP.hp = data.hpLeft || 0;
...
function submitChallenge() {
  if (APP.answered) { const data = DB.getDay(APP.currentDay).data; if (data) showResult(APP.currentDay, data.score, data.hpLeft, data.answered); return; }
...
function showResult(day, correct, hpLeft, answered) {
  ...
  if (passed) {
    btn.textContent = "🗺️ 返回冒险版图"; btn.className = "btn btn-success";
    btn.onclick = () => { overlay.classList.remove("show"); renderHome(); showPage("home"); };
  } else {
    btn.textContent = "🔄 重新挑战"; btn.className = "btn btn-gold";
